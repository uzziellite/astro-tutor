<script>
	...
	$: {
		...
		if(password !== confirm_password){
			...
			error_bag.password = "Password should be at least eight characters"
		}else{
		...
		<form on:submit|preventDefault={register}>
			<div class="grid sm:grid-cols-2 gap-2">
				<div class="space-y-1 text-sm">
					...
					<input type="text" id="first_name" placeholder="James" class="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-indigo-400" bind:value={first_name} required autofocus>
				...
				<div class="space-y-1 text-sm">
					...
					<input type="text" id="last_name" placeholder="Wafula" class="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-indigo-400" bind:value={last_name} required>
			...
			{#if !registering}
				<button class="block w-full p-3 text-center rounded-sm dark:text-gray-900 dark:bg-indigo-400 mt-4">
					Register
				</button>
			{:else}
